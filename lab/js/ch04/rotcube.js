...
window.onload = function initWindow() {
	...
	xmove.onchange = function(){
		move[0] = xmove.value/10;
	}
	ymove.onchange = function(){
		move[1] = ymove.value/10;
	}
	zmove.onchange = function(){
		move[2] = zmove.value/10;
	}
		
	xzoom.onchange = function(){
		zoom[0] = xzoom.value/10;
	}
	yzoom.onchange = function(){
		zoom[1] = yzoom.value/10;
	}
	zzoom.onchange = function(){
		zoom[2] = zzoom.value/10;
	}
...
function moveMotion(x, y){
	...
	if(trackingMouse){
		...
		if(dx||dy||dz){
			...
			axis02[0] = lastPos[1] * curPos[2] - lastPos[2] * curPos[1];
			axis02[1] = lastPos[2] * curPos[0] - lastPos[0] * curPos[2];
			axis02[2] = lastPos[0] * curPos[1] - lastPos[1] * curPos[0];
...
function render(){
	...
	if(trackballMove){
		glMatrix.vec3.normalize(axis02, axis02);
		var cosa = Math.cos(angle/2.0);
		var sina = Math.sin(angle/2.0);
		
		var rotation = glMatrix.vec4.fromValues(cosa, sina*axis02[0], sina*axis02[1], sina*axis02[2]);
